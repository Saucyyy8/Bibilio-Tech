...
<head>
    ...
    <title>Search Results - RNSIT Library</title>
...
<body class="bg-gray-50">
    ...
    <div class="container mx-auto px-4 py-8">
        <div class="mb-8">
            ...
            <c:if test="${not empty query || not empty category}">
                <p class="text-gray-600">
                    <c:if test="${not empty query}">Searching for: "${query}"</c:if>
                    <c:if test="${not empty query && not empty category}"> in </c:if>
        ...
        <c:choose>
            <c:when test="${empty books}">
                <div class="bg-white rounded-lg shadow-md p-8 text-center">
                    <h2 class="text-2xl font-bold text-gray-800 mb-4">No Books Found</h2>
                    <p class="text-gray-600 mb-4">We couldn't find any books matching your search criteria.</p>
                    <a href="/" class="bg-blue-800 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded-lg transition duration-300">Return Home</a>
                </div>
            </c:when>
            <c:otherwise>
